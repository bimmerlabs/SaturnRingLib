...
MU_TEST(dsp_test_load_start_and_end)
{
    ...
    SRL::SCU::DSP::Stop();
    static_cast<void>(SRL::SCU::DSP::CheckEnd());

        // Load an immediate-end program at PC=0
    constexpr uint16_t programLength = static_cast<uint16_t>(sizeof(DspProgramEnd) / sizeof(DspProgramEnd[0]));
    SRL::SCU::DSP::LoadProgram(0x00, DspProgramEnd, programLength);
...
MU_TEST(dsp_test_write_read_data_roundtrip)
{
    ...
    constexpr size_t wordCount = sizeof(inWords) / sizeof(inWords[0]);
    uint32_t outWords[wordCount] = {};

    SRL::SCU::DSP::WriteData(address, inWords, static_cast<uint16_t>(wordCount));
    SRL::SCU::DSP::ReadData(outWords, address, static_cast<uint16_t>(wordCount));

    for (uint16_t i = 0; i < static_cast<uint16_t>(wordCount); ++i)
    {
        snprintf(buffer, buffer_size, "DSP RAM mismatch at %u: 0x%08lx != 0x%08lx",
            static_cast<unsigned int>(i),
            static_cast<unsigned long>(outWords[i]),
            static_cast<unsigned long>(inWords[i]));
...
{
    SRL::SCU::DSP::Stop();
    mu_assert_int_eq(0, static_cast<int>(ReadRegister32(SRL::SCU::DSP::RegisterMap::RwCtrl)));
...
MU_TEST(dsp_test_check_end_not_ended)
{
    ...
    mu_assert_int_eq(static_cast<int>(SRL::SCU::DSP::EndState::NotEnded), static_cast<int>(state));
}
